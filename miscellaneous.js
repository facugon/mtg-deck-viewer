...
function OpenTip(event, obj)
{
	...
	if (isCardWide(card)) { cardClass += "wide"; }

	if (isCardWide(card) || card.urls.length > 1) { tipClass += "wide"; }
	...
	var imgSrc;
	if (card.imageUrl != null) { imgSrc = card.imageUrl; }
	else { imgSrc = imageHandler + card.urls[0] + card.options; }

	var tipTag = '<div style="width:265px;height:370px" class="' + tipClass + '"><img class="' + cardClass + '" src="' + imgSrc + '" />';
